...
def upgrade() -> None:
    ...
    op.create_index(
        'ix_raw_events_processing_status', 'raw_events', ['processing_status']
    )
    ...
    op.create_table(
        ...
        sa.Column(
            'events_count', sa.BigInteger(), server_default=sa.text('0'), nullable=False
        ),
        sa.Column(
            'page_views_count',
            sa.BigInteger(),
        ...
        sa.Column(
            'orders_count', sa.BigInteger(), server_default=sa.text('0'), nullable=False
        ),
        sa.Column(
            'total_revenue',
            sa.Numeric(18, 2),
        ...
        sa.Column(
            'currency', sa.String(), server_default=sa.text("'RUB'"), nullable=False
        ),
        ...
        sa.CheckConstraint(
            'events_count >= 0', name='ck_customer_profiles_events_count'
        ),
        sa.CheckConstraint(
            'page_views_count >= 0', name='ck_customer_profiles_page_views_count'
        ),
        sa.CheckConstraint(
            'cart_adds_count >= 0', name='ck_customer_profiles_cart_adds_count'
        ),
        sa.CheckConstraint(
            'orders_count >= 0', name='ck_customer_profiles_orders_count'
        ),
        sa.CheckConstraint(
            'total_revenue >= 0', name='ck_customer_profiles_total_revenue'
        ),
    )
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            'identity_type', 'identity_value', name='uq_identity_links_type_value'
        ),
        sa.CheckConstraint(
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            'source', 'order_id', name='uq_processed_purchases_source_order_id'
        ),
    ...
    op.create_index(
        'ix_processed_purchases_customer_id', 'processed_purchases', ['customer_id']
    )
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            'segment_id', 'customer_id', name='uq_segment_memberships_segment_customer'
        ),
    )
    op.create_index(
        'ix_segment_memberships_customer_id', 'segment_memberships', ['customer_id']
    )
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "destination_type = 'webhook'", name='ck_activation_jobs_destination_type'
        ),
        sa.CheckConstraint(
        ...
        sa.CheckConstraint(
            'members_count >= 0', name='ck_activation_jobs_members_count'
        ),
    )
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            'job_id', 'attempt_number', name='uq_activation_deliveries_job_attempt'
        ),
        sa.CheckConstraint(
            'attempt_number = 1', name='ck_activation_deliveries_attempt_number'
        ),
        sa.CheckConstraint(
