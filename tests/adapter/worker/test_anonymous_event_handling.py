...
@pytest.mark.asyncio
async def test_anonymous_event_is_ignored_correctly(router, mock_service, mock_producer):
...
@pytest.mark.asyncio
async def test_anonymous_event_does_not_trigger_dlq_on_success(router, mock_service, mock_producer):
    message = MagicMock()
    message.topic = "cdp.events.v1"
    message.value = json.dumps({
        "event_id": "evt_anon",
