    ...
    msg = create_kafka_msg(
        {
            ...
            'payload': {'order_id': 'order_123', 'amount': 1500.50, 'currency': 'RUB'},
        }
    )
...
@pytest.mark.asyncio
async def test_process_purchase_invalid_payload_goes_to_dlq(
    ...
    mock_service.process_event.return_value = ProcessEventResult.send_to_dlq(
        'evt_bad_p', 'invalid_purchase_payload'
    )
    ...
    assert (
        ...
        in mock_producer.send_and_wait.call_args[1]['value'].decode()
    )


@pytest.mark.asyncio
async def test_process_identity_conflict_goes_to_dlq(
    router, mock_service, mock_producer
):
    # Arrange
    msg = create_kafka_msg(
    ...
    mock_service.process_event.return_value = ProcessEventResult.send_to_dlq(
        'evt_conflict', 'identity_conflict'
    )
