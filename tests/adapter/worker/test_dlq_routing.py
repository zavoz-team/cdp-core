...
from adapter.worker.handlers import (
    EventMapper,
...
@pytest.mark.asyncio
async def test_dlq_message_content_on_identity_conflict(
...
async def test_dlq_message_content_on_mapping_error(router, mock_producer):
    ...
    dlq_payload = json.loads(
        mock_producer.send_and_wait.call_args[1]['value'].decode('utf-8')
    )
...
@pytest.mark.asyncio
async def test_offset_not_committed_if_dlq_publish_fails(
    router, mock_service, mock_producer
):
    # Arrange
    message = MagicMock()
